...
public class SeaRecipeProvider extends RecipeProvider {
    ...
    private static void seaGlassBlockset(ItemLike dye, Block pebbledBlock, Block bricks, Block brickSlab, Block brickStairs, Block seaGlass, Block seaGlassPane, Block lamp, Block nettedLamp, Block mosaicLamp, Consumer<FinishedRecipe> consumer) {
        ShapedRecipeBuilder.shaped(BUILDING_BLOCKS, pebbledBlock, 1).define('#', SeaItems.SEA_GLASS_PEBBLES.get()).define('D', dye).pattern("###").pattern("#D#").pattern("###").unlockedBy(getHasName(SeaItems.SEA_GLASS_PEBBLES.get()), has(SeaItems.SEA_GLASS_PEBBLES.get())).save(consumer, getSaveLocation(pebbledBlock));
