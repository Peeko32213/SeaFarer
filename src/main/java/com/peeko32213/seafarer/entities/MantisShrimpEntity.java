...
public class MantisShrimpEntity extends StatedWaterAnimal implements GeoAnimatable, GeoEntity {
    ...
    private static final EntityDataAccessor<Boolean> MANTIS_IDLE_SHAKE = SynchedEntityData.defineId(MantisShrimpEntity.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> MANTIS_IDLE_MOVE_EYES = SynchedEntityData.defineId(MantisShrimpEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public MantisShrimpEntity(EntityType<? extends WaterAnimal> pEntityType, Level pLevel) {
    ...
    protected void registerGoals() {
        this.goalSelector.addGoal(1, new MantisShrimpEntity.WanderGoal());
    ...
    protected <E extends MantisShrimpEntity> PlayState controller(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
    ...
    protected <E extends MantisShrimpEntity> PlayState attackController(final software.bernie.geckolib.core.animation.AnimationState<E> event) {
        if (this.swinging) {
    ...
    private class WanderGoal extends Goal {
        ...
        public boolean canUse() {
            if (MantisShrimpEntity.this.getRandom().nextInt(50) != 0) {
                return false;
            }
        ...
        public boolean canContinueToUse() {
            double dist = MantisShrimpEntity.this.distanceToSqr(x, y, z);
            return dist > 4F;
        }

        public void tick() {
            MantisShrimpEntity.this.getNavigation().moveTo(this.x, this.y, this.z, 1F);
        }

        public BlockPos findWaterBlock() {
            BlockPos result = null;
            RandomSource random = MantisShrimpEntity.this.getRandom();
            int range = 10;
            for (int i = 0; i < 15; i++) {
                BlockPos blockPos = MantisShrimpEntity.this.blockPosition().offset(random.nextInt(range) - range / 2, random.nextInt(range) - range / 2, random.nextInt(range) - range / 2);
                if (MantisShrimpEntity.this.level().getFluidState(blockPos).is(FluidTags.WATER) && blockPos.getY() > level().getMinBuildHeight() + 1) {
                    result = blockPos;
                }
            }
            return result;
        ...
        protected Vec3 getPosition() {
            ...
            if (MantisShrimpEntity.this.isInWaterOrBubble()) {
                if (water == null) {
                    return null;
                }
                while (MantisShrimpEntity.this.level().getFluidState(water.below()).is(FluidTags.WATER) && water.getY() > level().getMinBuildHeight() + 1) {
                    water = water.below();
                }
                water = water.above();
                return Vec3.atCenterOf(water);
            } else {
                return water == null ? DefaultRandomPos.getPos(MantisShrimpEntity.this, 7, 3) : Vec3.atCenterOf(water);
